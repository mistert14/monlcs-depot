...
define("MAGPIE_CACHE_AGE",0);	// Cache time in seconds- 10 minutes
...
$rss = fetch_rss(urldecode($_GET['rssURL']));
...
for($no=0;$no<count($rssItems);$no++){
	...
	//
	$desc = $rssItems[$no]["description"];
	if (eregi('src=',$desc)) {
	$desc = substr($desc,strpos($desc,'src=')+5);
	$fin = strpos($desc,'alt=')-2;
	$desc = substr($desc,0,$fin);
	$rssItems[$no]["enclosure"]=$desc;
	}
	...
	$outputItems[$key] = array(
		...
		"enclosure"=>$rssItems[$no]["enclosure"]);	
}
